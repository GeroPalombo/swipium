...
function compiledFlowIdentity(c: {
  name: string;
  ok: boolean;
  flowPath?: string;
  compiledPath?: string;
...
// ---- qa_generate target:"pom" core ----

export interface PomGenerateArgs {
  sessionId: string;
  name?: string;
  save?: boolean;
}
...
// ---- qa_generate target:"suite" core ----

export interface SuiteGenerateArgs {
  sessionId: string;
  name?: string;
  save?: boolean;
  compile?: boolean;
  replay?: 'none' | 'dry_run' | 'same_session' | 'fresh_state';
  stateProfile?: string;
  consentId?: string;
  approve?: boolean;
}
...
export async function runSuiteGenerate(
  ...
  const replayEvidence: {
    stateProfile?: string;
    stateProfilePath?: string | null;
    stateLedgerUris: string[];
  ...
  if (replayMode === 'dry_run') {
    for (const c of res.compiledFlows)
      replayResults.push({
        name: c.name,
        mode: replayMode,
        status: c.ok ? 'validated' : 'failed',
        reason: c.errors.join('; ') || undefined,
      });
    ...
    if (!driver)
      replayResults.push({
        ...
        mode: replayMode,
        status: 'blocked',
        reason: 'no driver attached for same-session replay',
      });
    else {
    ...
    if (!stateProfile)
      replayResults.push({
        ...
        mode: replayMode,
        status: 'blocked',
        reason: 'stateProfile is required for fresh_state replay',
      });
    else if (!driver)
      replayResults.push({
        name: res.name ?? 'suite',
        mode: replayMode,
        status: 'blocked',
        reason: 'no driver attached for fresh-state replay',
      });
    else {
      ...
      if (!loaded.profile)
        replayResults.push({
          ...
          mode: replayMode,
          status: 'blocked',
          reason: loaded.error ?? 'could not load state profile',
        });
      else {
        replayEvidence.stateProfile = loaded.profile.name;
        replayEvidence.stateProfilePath = loaded.path ?? null;
        const mutatingProfile =
          hasStateProfileMutation('prepare', loaded.profile, session) || hasStateProfileMutation('teardown', loaded.profile, session);
        ...
        if (mutatingProfile) {
          ...
          const risk =
            stateRisk('prepare', loaded.profile, session) === 'high' || stateRisk('teardown', loaded.profile, session) === 'high'
              ? 'high'
          ...
          if (!gate.approved) {
            ...
            return requireConsent({
              action: 'suite_fresh_state_replay',
              risk,
              exactCommand: `qa_generate target=suite replay=fresh_state stateProfile=${loaded.profile.name}`,
              affects,
        ...
        const saveLedger = (action: string, ledger: unknown) => {
          const uri = sessions.saveArtifact(
            session,
            'state',
            `suite-${action}-${loaded.profile!.name}-${Date.now()}.json`,
            JSON.stringify(ledger, null, 2),
            'application/json',
            `suite fresh-state ${action} ledger`,
          );
          ledgers.push(uri);
        ...
        if (prepared.status === 'state_refused' || prepared.status === 'state_blocked') {
          replayResults.push({
            ...
            mode: replayMode,
            status: 'blocked',
            reason: `state prepare ${prepared.status}; ledgers=${ledgers.join(',')}`,
          });
        } else {
          ...
          if (verified.status === 'state_blocked') {
            replayResults.push({
              ...
              mode: replayMode,
              status: 'blocked',
              reason: `state verify blocked; ledgers=${ledgers.join(',')}`,
            });
          } else {
            for (const c of res.compiledFlows.filter((f) => f.ok && f.flowPath)) {
              const parsed = parseFlow(readFileSync(c.flowPath!, 'utf8'));
  ...
  const ciReadyGate = {
    ...
    stableEvidence:
      ...
      replayResults.every((r) => r.status === 'passed') &&
      replayEvidence.stateLedgerUris.length >= 2,
  };
      ...
      JSON.stringify(
        {
          ...
          ciReady,
          ciReadyGate,
          compiledFlows: res.compiledFlows.map((f) => ({
            name: f.name,
            slug: f.slug,
            ok: f.ok,
            errors: f.errors,
  ...
  return qaOk(
    {
      ...
      compiledFlows: res.compiledFlows,
      suiteRunnable: res.suiteRunnable,
      replay: { mode: replayMode, results: replayResults, passed: replayPassed, evidence: replayEvidence },
      readiness,
      readinessLabels,
...
// ---- qa_generate target:"testcases" core ----

export interface TestcaseGenerateArgs {
  sessionId: string;
  name?: string;
...
export async function runTestcaseGenerate(
  ...
  return qaOk(
    { cases: tc.cases, yaml: fmt !== 'markdown' ? tc.yaml : undefined, markdown: fmt !== 'yaml' ? tc.markdown : undefined, written },
    summary,
  );
}
  ...
  server.registerTool(
    ...
    async ({ sessionId, projectRoot, suite }) => {
      ...
      if (result.errors.length && result.flows.length === 0) {
        return qaError({
          what: `Could not compile suite: ${result.errors.join('; ')}`,
          changedState: false,
          retrySafe: true,
          nextSteps: ['Generate a suite first: qa_generate target:"suite", then qa_flow_compile.'],
        });
      }
