...
export function agentBrief() {
  return {
    ...
    goals: TEST_GOALS,
    needsInputRule:
      "Relay exactly the one returned question (with its fields/secret flags). Do not invent extra questions. Resume with the response's `resume` call.",
    blockerRule:
      'Relay failureCode, owner, what Swipium tried (attempted/workaroundsAttempted), and how to fix it. A build failure is not a test failure.',
    stopRule:
      'Stop and ask the user only on a needs_input state (credentials, monorepo target, destructive approval, signing, external service). Otherwise continue without user input.',
    continueRule:
      'For completed/blocked/unsafe states, summarize from the structured envelope and the report — no extra Swipium calls are required.',
    mapRule:
      'Before feature-focused testing, consult the durable App Knowledge Map: qa_app_map_read, qa_app_map_query, or qa_app_map_feature_scope to reuse what Swipium already learned instead of rediscovering the app. qa_test_this and qa_explore update the map automatically.',
...
export function registerAgentTools(server: McpServer, sessions: SessionStore): void {
  ...
  server.registerTool(
    'qa_agent_brief',
    {
      ...
      description:
        'Read-only, cheap, deterministic. Returns the current-version instruction set for coding agents: the first call for low-context QA (qa_test_this), the polling + report-fetch flow, the NeedsInput/blocker rules, when to stop vs. continue, and which tools are macros vs. low-level escape hatches. Call this once at the start instead of inspecting the source tree. Orientation: this is the FIRST call / how-to; qa_capabilities is the full tool catalog by group; qa_status is the current session/device state; qa_next_best_action is the single next tool to call.',
      inputSchema: {},
    },
    async () => {
      const b = agentBrief();
  ...
  server.registerTool(
    'qa_status',
    {
      ...
      description:
        'A concise snapshot of a session for an agent: project root/framework, bound device + app, budget remaining, counters, recorded actions, findings/notes, last job, and the safe fallbacks taken (workarounds). Read-only, cheap — call between steps instead of re-deriving state. Orientation: this is the current session/device STATE; qa_agent_brief is the first-call how-to; qa_capabilities is the tool catalog by group; qa_next_best_action is the single next tool to call.',
  ...
  server.registerTool(
    'qa_next_best_action',
    {
      ...
      description:
        'Given the current session state, returns the single best next tool to call (with args) and why — so an agent does not have to reason out the orchestration sequence. Deterministic, read-only. Pass `goal` to bias the first call toward a specific autopilot intent. Orientation: this answers "what do I do NOW"; qa_agent_brief is the first-call how-to; qa_capabilities is the tool catalog by group; qa_status is the current session/device state.',
      inputSchema: {
        sessionId: z.string(),
        goal: z.enum(['smoke', 'explore', 'create_automation_suite', 'release_gate', 'test_login', 'reproduce_bug']).optional(),
      },
    },
