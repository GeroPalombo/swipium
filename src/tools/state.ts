...
function seedActionPreview(seed: FixtureSeedAction | undefined): Record<string, unknown> | null {
  ...
  if (seed.type === 'api')
    return {
      ...
      url: seed.url ?? null,
      bodyHash: seed.body ? hash(seed.body).slice(0, 16) : null,
      headerKeys: Object.keys(seed.headers ?? {}).sort(),
    };
...
function cleanupFixtures(session: Session | undefined, profile: StateProfile): string[] {
  if (!session) return [];
  return (profile.seed ?? []).map((s) => s.fixture).filter((name) => !!session.fixtures.find((f) => f.name === name)?.seed?.cleanup);
}
...
export function stateProfileAffects(
  session: Session,
  action: 'prepare' | 'verify' | 'teardown',
...
export function stateRisk(action: 'prepare' | 'verify' | 'teardown', profile: StateProfile, session: Session): 'low' | 'medium' | 'high' {
  ...
  const cleanupTypes =
    action === 'teardown' ? (profile.seed ?? []).map((s) => session.fixtures.find((f) => f.name === s.fixture)?.seed?.cleanup?.type) : [];
...
export function registerState(server: McpServer, sessions: SessionStore): void {
  const handler =
    (action: 'prepare' | 'verify' | 'teardown') =>
    async ({ sessionId, profile, consentId, approve }: { sessionId: string; profile: string; consentId?: string; approve?: boolean }) => {
      ...
      if (!loaded.profile)
        return qaError({
          what: loaded.error ?? 'Could not load state profile',
          changedState: false,
          retrySafe: true,
          failureCode: 'INVALID_FLOW',
          nextSteps: ['Create .swipium/state/<name>.yaml or pass inline YAML.'],
        });
      ...
      const uri = sessions.saveArtifact(
        session,
        'state',
        `state-${action}-${loaded.profile.name}-${Date.now()}.json`,
      ...
      if (mutating)
        sessions.recordMutation(session, {
          ...
          target: affects,
          consent: { required: true, consentId, approved: true, payloadHash: mutationPayloadHash },
          status: ok ? 'executed' : ledger.status === 'state_refused' ? 'refused' : 'blocked',
          ledgerUri: uri,
      ...
      return qaOk(
        { action, profile: loaded.profile.name, path: loaded.path ?? null, ledger, ledgerUri: uri, ok },
        `state ${action} ${loaded.profile.name}: ${ledger.status}\nledger: ${uri}`,
      );
    };
  ...
  server.registerTool(
    'qa_state_prepare',
    {
      ...
      description:
        'Apply a .swipium/state/*.yaml profile as one transaction: reset policy, launch, seed fixtures, and ledger artifact. Refuses debug-bundle-loss resets unless explicitly acknowledged.',
  ...
  server.registerTool(
    'qa_state_verify',
    {
      ...
      description:
        "Check that a state profile's declared checks (e.g. assertVisible) hold on the CURRENT screen — USE WHEN you must confirm a precondition before testing, or re-confirm it after a run, without re-applying the profile. A failed check is a setup/state blocker (failureCode STATE_*), NOT an app test failure — do not report it as an app bug. Preconditions: a session with an attached device, and a .swipium/state/*.yaml profile (by name, path, or inline YAML). Read-only, no consent needed. Returns { action, profile, ledger, ledgerUri, ok } — the ledger lists each check's outcome.",
      inputSchema: schema,
    },
    handler('verify'),
  );
  ...
  server.registerTool(
    'qa_state_teardown',
    {
      ...
      description:
        "Run a state profile's declared teardown steps (e.g. restore networkOnline, fixture cleanup) and write a transaction ledger artifact. USE WHEN a run that used qa_state_prepare (or otherwise mutated declared state) is finished and the device/app must be left clean. Preconditions: a session with an attached device and the same profile that was prepared. Mutating → consent-gated (returns a consent request first; re-call with consentId + approve:true). Returns { action, profile, ledger, ledgerUri, ok }; ok:false carries a failureCode when a step was refused or blocked.",
      inputSchema: mutatingSchema,
    },
    handler('teardown'),
  );
