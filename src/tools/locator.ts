    ...
    async ({ sessionId, interactiveOnly }) => {
      ...
      if (driver.kind === 'simulator' || session.mode === 'visual-fallback') {
        return qaError({
          ...
          nextSteps: ['On the iOS simulator or a visual-only screen there is no a11y tree to score. Use qa_visual / qa_assert_visual instead.'],
        });
      }
      ...
      try {
        ...
        return qaError({ what: `Could not read the UI tree: ${String(e)}`, changedState: false, retrySafe: true, failureCode: 'SNAPSHOT_FAILED', nextSteps: ['Retry on a settled screen.'] });
      }
      ...
      const summary =
        ...
        `platform: ${platform}\n` +
        `locator tiers: ${Object.entries(tierTally).map(([t, n]) => `${t}=${n}`).join(' ')}\n` +
        (issues.length ? `issues:\n${issues.slice(0, 8).map((i) => `  - ${i.code}: ${i.message}`).join('\n')}\n` : '') +
        (readiness.needTestIds.length
          ? `needs a testID (${readiness.needTestIds.length}):\n` + readiness.needTestIds.slice(0, 15).map((n) => `  - ${n.ref} [${n.role}] — ${n.hint}${n.suggestedTestId ? `; suggested ${platform === 'ios' ? 'accessibilityIdentifier' : 'testID'}: ${n.suggestedTestId}` : ''}`).join('\n')
