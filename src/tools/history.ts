  ...
  server.registerTool(
    'qa_report_compare',
    {
      ...
      inputSchema: {
        ...
        trendRoot: z
          .string()
          .optional()
          .describe('Optional project root containing .swipium/runs or legacy .swipium/ci history for known-flaky classification.'),
      },
    },
    async ({ current, baseline, trendRoot }) => {
      try {
        ...
        return qaError({
          what: `Could not compare reports: ${String((e as Error).message ?? e)}`,
          changedState: false,
          retrySafe: true,
          nextSteps: ['Pass readable Swipium report.json files for current and baseline.'],
        });
      }
    },
  );
    ...
    async ({ sessionId, projectRoot }) => {
      ...
      if (sessionId && !session)
        return qaError({
          ...
          changedState: false,
          retrySafe: true,
          nextSteps: ['Call qa_start_session first, or pass projectRoot.'],
        });
      const root = projectRoot ?? session?.root;
      if (!root)
        return qaError({
          what: 'qa_run_history needs projectRoot or sessionId',
          changedState: false,
          retrySafe: true,
          nextSteps: ['Pass projectRoot, or call qa_start_session first.'],
        });
      try {
        ...
        const asSec = (ms: number | null): string => (ms == null ? 'n/a' : `${Math.round(ms / 100) / 10}s`);
        const summary =
          `history: ${trend.reports} report(s), median ${asSec(trend.medianRuntimeMs)}\n` +
          `timing: avg setup ${asSec(trend.averageSetupMs)}, avg active ${asSec(trend.averageActiveMs)}\n` +
          (locatorTrend.runsWithReadiness
            ? `locator readiness: latest durable ${locatorTrend.latestDurablePct ?? 'n/a'}%, average ${locatorTrend.averageDurablePct ?? 'n/a'}%\n`
            : '') +
          Object.entries(trend.passRate)
            .map(([flow, rate]) => `  ${flow}: ${(rate * 100).toFixed(1)}% pass`)
            .join('\n') +
          ...
          (trend.slowestSteps.length
            ? `\nslowest step: ${trend.slowestSteps[0].workflow}#${trend.slowestSteps[0].index} ${trend.slowestSteps[0].kind} ${asSec(trend.slowestSteps[0].durationMs)}`
            : '');
        return qaOk({ root, ...trend }, summary);
      } catch (e) {
        return qaError({
          what: `Could not read run history: ${String((e as Error).message ?? e)}`,
          changedState: false,
          retrySafe: true,
          nextSteps: ['Confirm .swipium/runs or .swipium/ci contains report.json files.'],
        });
      }
    },
  );
