 ...
 *  Shared by qa_test_feature (below) and qa_app_map_feature_scope (src/tools/appMap.ts). */
export async function resolveFeatureContext(
  ...
  const session = args.sessionId ? sessions.get(args.sessionId) : undefined;
  ...
  const scopeResult = buildMapFeatureScope({
    root,
    query: args.feature,
    index,
    runtimeScreens,
...
export function registerFeatureTesting(server: McpServer, sessions: SessionStore): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      // PLAN mode: pure, read-only planning under the action tool name.
      if (effectiveMode === 'plan') {
        const r = await resolveFeatureContext(server, sessions, { sessionId, projectRoot, feature, platform, includeCode, limit });
        if (!r.ok) return r.result;
        const { scopeResult, objective } = r.ctx;
        if (!scopeResult.found)
          return qaOk(
            { sessionId: sessionId ?? null, feature, found: false, searched: scopeResult.searched },
            `No feature matched "${feature}".`,
          );
        ...
        const plan = buildFeatureTestPlan(scopeResult.primary, objective, {
        ...
        return qaOk(
          { sessionId: sessionId ?? null, feature, mode: 'plan', found: true, plan },
          `📋 ${plan.title}: ${plan.cases.length} case(s), readiness ${plan.automationReadiness}. To run it: qa_test_feature { mode:"execute" }.`,
        );
      }
      ...
      return qaOk(
        {
          ...
          kind: job.kind,
          feature,
          featureId: scopeResult.primary.featureId,
...
async function runFeatureTestJob(sessions: SessionStore, session: Session, job: JobRecord, a: FeatureTestJobArgs): Promise<void> {
  ...
  const prog = startProgress(sessions, session, job, 'exploring', {
    statusText: `Focusing tests on "${a.scopeResult.primary.title}".`,
    nextExpected: 'Navigate to the feature and exercise it.',
  });
  ...
  try {
    ...
    const graphUri = sessions.saveArtifact(
      session,
      'explore',
      `feature-graph-${at}.json`,
      JSON.stringify(serialized, null, 2),
      'application/json',
      `feature graph: ${scope.title}`,
    );
    ...
    const rescoped = buildFeatureScope({
      query: a.feature,
      index: { root: session.root, symbols: [], routes: [], files: [], scannedFiles: 0, truncated: false },
      runtimeScreens,
      existingTests: scope.existingTests,
      platform: a.platform,
    });
    ...
    const cases = a.generateCases
      ? generateFeatureTestCases(liveScope, a.objective, {
          creativity: a.creativity,
          platform: a.platform,
          allowAdversarial: a.allowAdversarial,
        })
      : [];
    const newNotes = session.notes.slice(notesBefore);
    const mergeNotes: MergeNote[] = newNotes.map((n) => ({
      workflow: n.workflow,
      outcome: n.outcome,
      reason: n.reason,
    ...
    const merge = mergeFeatureRun(
      ...
      {
        notes: mergeNotes,
        visitedRuntimeScreens: visited,
        evidence: [graphUri],
        exploration: { state: ex.state, stoppedReason: ex.stoppedReason, screensVisited: ex.summary.screensVisited },
        blocked,
        blockReason,
        blockGuidance: ex.needsInput?.ifDeclined,
      },
      prior,
    );
    ...
    const casesUri = sessions.saveArtifact(
      session,
      'feature',
      `cases-${liveScope.featureId}-${at}.json`,
      JSON.stringify({ featureId: liveScope.featureId, title: liveScope.title, objective: a.objective, cases: merge.cases }, null, 2),
      'application/json',
      `feature cases: ${liveScope.title}`,
    );
    ...
    try {
      ...
      if (canonical.length) {
        const applied = applyMerge(
          session.root,
          canonical,
          { source: 'feature', mode: 'update', now: generatedAt, runId: `feature-${at}`, sourceUri: casesUri },
          session.appId,
        );
    ...
    const nextRecommendedAction = blocked
      ? {
          ...
          why: 'Provide setup to unblock the feature, then re-run qa_test_feature',
        }
      : reportUri
