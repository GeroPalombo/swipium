...
import { checkWda, classifyWdaBuildFailure, classifyWdaConnectionFailure, createWdaSession, discoverWdaProjects, managedWdaBuildArgs, managedWdaStartArgs, waitForWdaReady, wdaSessionUdidMismatch, xcodeAvailable } from '../lib/wda.js';
...
function latestWdaArtifacts(session: Session): { latestLog: ArtifactRecord | null; latestBuildLog: ArtifactRecord | null; latestStartLog: ArtifactRecord | null; latestErrorLog: ArtifactRecord | null } {
...
function issue(code: string, detail: string, nextStep: string, severity: WdaDiagnosticIssue['severity'] = 'blocker', failureCode?: string): WdaDiagnosticIssue {
  ...
  server.registerTool(
    'qa_wda',
    {
      ...
      inputSchema: {
        ...
        webDriverAgentUrl: z.string().optional().describe('External WDA base URL. Defaults to http://127.0.0.1:8100. Non-loopback URLs are refused by default.'),
        udid: z.string().optional().describe('Simulator/device UDID expected behind this WDA endpoint.'),
        bundleId: z.string().optional().describe('App bundle id for session creation. Defaults to the session appId.'),
        wdaProjectPath: z.string().optional().describe('Path to WebDriverAgent.xcodeproj for managed build/start. Relative paths resolve from project root.'),
        derivedDataPath: z.string().optional().describe('Optional xcodebuild -derivedDataPath for WDA caching/reuse. Relative paths resolve from project root.'),
    ...
    async ({ sessionId, action, webDriverAgentUrl, udid, bundleId, wdaProjectPath, derivedDataPath, scheme, allowNonLoopback, consentId, approve }) => {
      ...
      if (action === 'build' || action === 'start') {
        if (!targetUdid) {
          return qaError({ what: `qa_wda ${action} requires a simulator/device UDID`, changedState: false, retrySafe: true, failureCode: 'NO_DEVICE', nextSteps: ['Boot/select a simulator with qa_ios boot, or pass udid explicitly.'] });
        }
        if (action === 'start' && configured.reuse) {
          ...
          if (existing.reachable && existing.ready) {
            return qaOk(
              { started: false, reused: true, ready: true, webDriverAgentUrl: url, udid: targetUdid, wda: existing, wdaConfig: configured, reuseCheckMs },
        ...
        if (!xcode.available) {
          return qaError({ what: 'Xcode command line tools are unavailable', changedState: false, retrySafe: true, failureCode: 'BACKEND_UNSUPPORTED', nextSteps: [xcode.error ?? 'Install Xcode and select it with xcode-select.'] }, { xcode });
        }
        if (!projectPath || !existsSync(projectPath)) {
          return qaError({ what: `qa_wda ${action} requires wdaProjectPath pointing to WebDriverAgent.xcodeproj`, changedState: false, retrySafe: true, failureCode: 'NO_ARTIFACT', nextSteps: ['Pass wdaProjectPath, for example path/to/WebDriverAgent.xcodeproj.'] }, { xcode, wdaProjectPath: projectPath ?? null });
        }
        const args = action === 'build'
          ? managedWdaBuildArgs({ projectPath, udid: targetUdid, derivedDataPath: ddPath, scheme, developmentTeam: configured.developmentTeam })
          : managedWdaStartArgs({ projectPath, udid: targetUdid, derivedDataPath: ddPath, scheme, developmentTeam: configured.developmentTeam });
        ...
        if (!gate.approved) {
          ...
          return requireConsent({ action: `wda_${action}`, risk: 'medium', exactCommand: `xcodebuild ${args.join(' ')}`, affects: { udid: targetUdid, projectPath }, explain: `${action === 'build' ? 'Build' : 'Start'} WebDriverAgent with xcodebuild? This can use local signing context and run for a while.` });
        }
        sessions.recordMutation(session, {
          tool: 'qa_wda',
          action: `wda_${action}`,
        ...
        if (action === 'build') {
          ...
          const log = `${r.stdout}\n${r.stderr}`;
          const logUri = sessions.saveArtifact(session, 'wda', `wda-build-${Date.now()}.log`, log, 'text/plain', `WDA build ${r.code === 0 ? 'success' : 'failed'}`);
          ...
          if (r.code !== 0) {
            ...
            return qaError({
              what: failureCode === 'WDA_SIGNING_FAILED' ? `WDA signing/provisioning failed with exit ${r.code}` : `WDA build failed with exit ${r.code}`,
              changedState: true,
              retrySafe: failureCode !== 'WDA_SIGNING_FAILED',
              failureCode,
              artifactUri: logUri,
              nextSteps: failureCode === 'WDA_SIGNING_FAILED'
          ...
          return qaOk({ built: true, logUri, xcode, command: ['xcodebuild', ...args], wdaConfig: configured }, `WDA build completed → ${logUri}`);
        }
        ...
        sessions.recordMutation(session, {
          ...
          action: 'wda_start',
          risk: 'medium',
          target: { udid: targetUdid, projectPath, derivedDataPath: ddPath, scheme: scheme ?? null, pid: child.pid ?? null, webDriverAgentUrl: url },
          consent: { required: true, consentId, approved: true },
          status: 'executed',
          ledgerUri: logUri,
        ...
        if (!waited.ready) {
          ...
          return qaError({
            ...
            artifactUri: logUri,
            nextSteps: ['Open the WDA start log artifact, fix the launch/signing/device issue, then retry qa_wda start. If WDA eventually becomes ready, qa_wda attach can still use this managed process.'],
          }, { started: true, ready: false, pid: child.pid ?? null, logUri, webDriverAgentUrl: url, command: ['xcodebuild', ...args], wdaConfig: configured, wda: waited.status, startupWaitMs: waited.durationMs });
        }
        return qaOk({ started: true, ready: true, pid: child.pid ?? null, logUri, webDriverAgentUrl: url, command: ['xcodebuild', ...args], wdaConfig: configured, wda: waited.status, startupWaitMs: waited.durationMs }, `started managed WDA pid ${child.pid ?? 'unknown'} and /status is ready → ${logUri}\nNext: qa_wda attach.`);
      }
      ...
      if (action === 'status' || action === 'doctor' || action === 'diagnose') {
        ...
        if (wantsManaged && process.platform !== 'darwin') issues.push(issue('HOST_UNSUPPORTED', 'managed WDA requires a macOS host', 'Run managed WDA on macOS, or provide an external WDA URL.'));
        if (wantsManaged && !xcode.available) issues.push(issue('XCODE_UNAVAILABLE', 'xcodebuild is unavailable', xcode.error ?? 'Install Xcode command line tools and select them with xcode-select.'));
        if (process.platform === 'darwin' && !simctlOk) issues.push(issue('SIMCTL_UNAVAILABLE', 'xcrun simctl is unavailable', 'Install/select Xcode command line tools, then retry qa_ios list or qa_wda doctor.'));
        if (!bootedUdid && simctlOk && simulators.length === 0) {
          issues.push(issue('IOS_SIMULATOR_RUNTIME_MISSING', 'xcrun simctl is available, but no iOS simulators were found', 'Install an iOS simulator runtime in Xcode, create an iPhone simulator, then retry qa_wda doctor.', 'blocker', 'SIMULATOR_RUNTIME_MISSING'));
        } else if (!bootedUdid && simctlOk && bootedSimulators.length === 0) {
          issues.push(issue('NO_BOOTED_SIMULATOR', 'no booted iOS simulator was detected', 'Boot/select a simulator with qa_ios boot, or pass udid for an already-running WDA target.'));
        }
        if (!bootedUdid) issues.push(issue('NO_UDID', 'no simulator/device UDID is bound to this session', 'Boot/select a simulator with qa_ios boot, or pass udid explicitly.'));
        if (wantsManaged && (!projectPath || !existsSync(projectPath))) {
          const discovered = wdaProjectDiscovery.candidates[0];
          issues.push(issue(
            'WDA_PROJECT_MISSING',
            discovered ? `managed WDA needs wdaProjectPath; discovered candidate ${discovered}` : 'managed WDA needs a WebDriverAgent.xcodeproj path; no local candidate was discovered',
            discovered ? `Pass wdaProjectPath: "${discovered}", or configure ios.wda for that checked-out WebDriverAgent project.` : 'Check out WebDriverAgent/Appium WDA, pass wdaProjectPath, or provide an external WDA URL.',
          ));
        }
        if (wantsManaged && projectPath && existsSync(projectPath) && wdaBuildProduct && !wdaBuildProduct.built) issues.push(issue('WDA_NOT_BUILT', `no WebDriverAgentRunner .app was found under derivedDataPath ${wdaBuildProduct.checkedPath}`, 'Run qa_wda build first, or point ios.wda.derivedDataPath at a cache containing a built WebDriverAgentRunner product.'));
        if (wantsManaged && !configured.developmentTeam) issues.push(issue('WDA_SIGNING_UNCONFIGURED', 'no development team is configured for managed WDA signing', 'Set ios.wda.developmentTeam in .swipium/config.json or DEVELOPMENT_TEAM/XCODE_DEVELOPMENT_TEAM in the environment.', 'warn'));
        if (!status.reachable) issues.push(issue('WDA_SERVER_UNAVAILABLE', `WDA server is unavailable at ${url}`, 'Start WebDriverAgent externally, or run qa_wda build/start for managed WDA.'));
        else if (!status.ready) issues.push(issue('WDA_NOT_READY', status.message ?? `WDA responded at ${url} but did not report ready`, 'Check WDA logs and wait/restart before attaching.', 'warn'));
        if (!appId) issues.push(issue('NO_BUNDLE_ID', 'no app bundle id is configured for WDA session creation', 'Set appId in .swipium/config.json, prepare the target, or pass bundleId.'));
        if (appInstalled === false) issues.push(issue('APP_NOT_INSTALLED', `bundle id ${appId} is not installed on ${bootedUdid}`, 'Install/launch the app with qa_ios or qa_prepare_target before attaching WDA.'));
        if (artifactSummary.latestErrorLog) {
          ...
          if (failureCode === 'WDA_SIGNING_FAILED') {
            issues.push(issue('WDA_SIGNING_FAILED', `latest WDA build log indicates signing/provisioning failed: ${artifactSummary.latestErrorLog.uri}`, 'Configure WDA signing/provisioning for the target device, then rerun qa_wda build.', 'blocker', failureCode));
          } else if (failureCode === 'WDA_BUILD_FAILED') {
            issues.push(issue('WDA_BUILD_FAILED', `latest WDA build log indicates xcodebuild failed: ${artifactSummary.latestErrorLog.uri}`, 'Open the WDA build log artifact, fix the Xcode build error, then rerun qa_wda build.', 'blocker', failureCode));
          }
          issues.push(issue('LAST_WDA_ERROR_LOG', `latest WDA error log artifact: ${artifactSummary.latestErrorLog.uri}${failureCode ? ` (${failureCode})` : ''}`, 'Open qa_wda logs or qa_get_artifact for the captured WDA failure log.', 'warn', failureCode));
        }
      ...
      if (!targetUdid) {
        return qaError({
          ...
          nextSteps: ['Pass udid explicitly, or bind the session to a device first. This prevents attaching to a stale WDA for the wrong device.'],
        }, base);
      }
      if (!status.reachable) {
        return qaError({
      ...
      try {
        const sessionOptions = { bundleId: appId ?? undefined, udid: targetUdid, capabilities: configured.capabilities, settings: configured.settings };
        ...
        const driver = new WdaDriver(url, { ...sessionOptions, sessionId: created.sessionId, onTiming: (kind, ms) => recordWdaTiming(session, kind, ms, sessions) });
        ...
        sessions.recordMutation(session, {
          ...
          action: 'wda_attach',
          risk: 'medium',
          target: { webDriverAgentUrl: url, udid: targetUdid, bundleId: appId ?? null, wdaSessionId: created.sessionId, capabilities: created.capabilities ?? null },
