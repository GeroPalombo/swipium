...
// qa_automation_generate emits an Appium POM suite, the app map should KNOW automation now exists for
...
import { loadAppMap, saveAppMap, appMapResourceUri } from './store.js';
...
export function linkAutomationSuite(root: string, suite: AutomationSuiteRef, now: string): LinkAutomationResult {
  ...
  addProvenance(map, makeProvenance('test_case', now, `Automation suite ${suite.name} linked (${suite.framework ?? 'appium'})`, { targetType: 'test', refs: [suite.path] }));
  map.updatedAt = now;
