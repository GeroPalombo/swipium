...
const FEATURE_RULES: Array<{ feature: FeatureKey; re: RegExp; task: string; risk: ExploreTask['risk'] }> = [
  {
    feature: 'navigation',
    re: /\b(home|tab|menu|back|next|details|open|view|dashboard|overview)\b/i,
    task: 'Exercise primary navigation',
    risk: 'safe',
  },
  {
    feature: 'auth',
    re: /\b(sign in|log in|login|password|email|account|register|create account)\b/i,
    task: 'Reach and validate authentication',
    risk: 'unknown',
  },
  ...
  {
    feature: 'delete',
    re: /\b(delete|remove|clear|wipe|discard)\b/i,
    task: 'Discover delete flow without executing it',
    risk: 'destructive',
  },
  {
    feature: 'share/send',
    re: /\b(share|send|invite|message|email)\b/i,
    task: 'Discover share or send flow without external side effects',
    risk: 'destructive',
  },
  ...
  { feature: 'media', re: /\b(photo|camera|image|video|gallery|media)\b/i, task: 'Exercise media surface', risk: 'unknown' },
  {
    ...
    re: /\b(map|location|nearby|route|gps|address)\b/i,
    task: 'Exercise map or location surface',
    risk: 'unknown',
  },
  {
    feature: 'purchase',
    re: /\b(pay|buy|purchase|checkout|subscribe|order)\b/i,
    task: 'Discover purchase flow without executing it',
    risk: 'destructive',
  },
...
export function inferAppDomain(input: { packageId?: string; goal?: string; screenText: string; fixtures: string[] }): {
  domain: string;
  evidence: string[];
} {
...
export function proposeTasks(input: {
  ...
  for (const rule of FEATURE_RULES) {
    ...
    if (rule.feature === 'auth' && !input.fixtures.some((f) => /account|login|auth|credential/i.test(f)))
      preconditions.push('test account credentials');
  ...
  if (!tasks.length) {
    tasks.push({
      id: 'task-1',
      title: input.goal || 'Explore primary reachable workflow',
      feature: 'navigation',
      preconditions: [],
      risk: 'safe',
      status: 'proposed',
...
export function estimateFeatureCoverage(nodes: ScreenNode[], tasks: ExploreTask[]): Record<FeatureKey, FeatureStatus> {
  ...
  for (const task of tasks) {
    if (task.status === 'blocked')
      out[task.feature] = task.preconditions.some((p) => /fixture|account|credential|state/i.test(p)) ? 'needs_fixture' : 'blocked';
...
export function coverageClaimsFrom(features: Record<FeatureKey, FeatureStatus>, nodes: ScreenNode[]): CoverageClaim[] {
  return FEATURE_KEYS.map((feature) => {
    ...
    const evidence = nodes
      .filter((n) =>
        n.elements.some((e) => `${e.label ?? ''} ${e.locator?.value ?? ''}`.toLowerCase().includes(feature.split('/')[0].split('-')[0])),
      )
      .map((n) => n.id);
    return {
      feature,
      ...
      reason:
        status === 'covered'
          ? 'reachable UI evidence was observed'
          : status === 'unsafe'
            ? 'only high-impact controls were found'
            : status === 'needs_fixture'
              ? 'requires declared test data or disposable state'
              : 'no strong feature signal observed',
    };
  });
}
...
export function reflectExploration(nodes: ScreenNode[], sameScreenEdges: number, reasons: string[]): ReflectionResult {
  ...
  return {
    ...
    reasons: [
      ...reasons,
      low.length
        ? `${low.length} screen(s) need better locators before CI promotion`
        : 'locator readiness is acceptable for candidate suite promotion',
