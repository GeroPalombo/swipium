...
const SEL_TO_PLATFORM: Record<TargetSelection, 'android' | 'ios'> = {
  'android-emulator': 'android', 'android-real': 'android', 'ios-simulator': 'ios', 'ios-real': 'ios',
};
...
export type BootstrapResult =
  | { ok: true; session: Session; driver: Driver }
...
export async function bootstrapFeatureExecution(a: BootstrapArgs): Promise<BootstrapResult> {
  ...
  if (!resolved.root) {
    return { ok: false, result: qaError({ what: 'Could not resolve a project root for feature execution', changedState: false, retrySafe: true, nextSteps: ['Pass projectRoot="/abs/path" or a sessionId from qa_test_this.'], clientHint: resolved.hint }) };
  }
  ...
  if (target.blocked) {
    return {
      ok: false,
      result: qaError(
        { what: target.blocked.detail, changedState: false, retrySafe: true, failureCode: target.blocked.failureCode, nextSteps: ['Bring a device online or create one (qa_doctor), then retry.', `Or run qa_test_this { projectRoot:"${root}", mode:"execute" }.`] },
        { sessionId: session.id },
      ),
    };
  }
  ...
  const preflight = buildTestThisPreflight({
    isAndroid, needBuild: false, willBoot: target.willBoot, bootTarget: target.bootTarget, isAab: false,
    apkPath: effectiveApk, externalApk, iosApp, iosAppOutsideRoot: iosApp ? !iosApp.startsWith(root + sep) : undefined, iosReal: false,
  });
    ...
    if (!gate.approved) {
      ...
      return {
        ...
        result: requireConsent({
          action: 'test_this_plan', risk: preflight.risk, exactCommand: preflight.exactCommand, affects: preflight.consentAffects,
  ...
  try {
    if (isAndroid) {
      ...
      if (!appId) return { ok: false, result: qaError({ what: 'Could not determine the app id from the APK.', changedState: false, retrySafe: false, failureCode: 'BUNDLE_ID_NOT_FOUND', nextSteps: ['Provide an APK with a readable manifest, or pass an explicit bundleId via qa_test_this.'] }, { sessionId: session.id }) };
      ...
      const res = await prepareAndroid(
        a.sessions, session, driver,
        { needBoot: target.willBoot, bootTarget: target.bootTarget, serial: target.device, resolvedAppId: appId, apk: effectiveApk, rnDebug: scan.metroNeed === 'likely', allowLaunchWithoutMetro: false, mutationConsent },
        { signal: a.signal },
      );
      if (!res.ok) return { ok: false, result: qaError({ what: `Device preparation failed: ${res.error ?? res.failureCode}`, changedState: true, retrySafe: true, failureCode: res.failureCode ?? 'APP_LAUNCH_FAILED', nextSteps: ['Resolve the blocker and retry, or run qa_test_this { mode:"execute" }.'] }, { sessionId: session.id }) };
    } else {
      const res = await prepareIos(a.sessions, session, { app: iosApp, bundleId: art.best.appId ?? undefined, simulator: target.device, attachWda: 'auto', mutationConsent }, {});
      if (!res.ok) return { ok: false, result: qaError({ what: `iOS preparation failed: ${res.error ?? res.failureCode}`, changedState: true, retrySafe: true, failureCode: res.failureCode ?? 'APP_LAUNCH_FAILED', nextSteps: ['Resolve the blocker and retry, or run qa_test_this { mode:"execute" }.'] }, { sessionId: session.id }) };
    }
  } catch (e) {
    return { ok: false, result: qaError({ what: `Device preparation error: ${String(e)}`, changedState: true, retrySafe: true, nextSteps: ['Retry, or run qa_test_this { mode:"execute" } to prepare a device.'] }, { sessionId: session.id }) };
  }
