...
export function toMarkdown(r: ReportData): string {
  ...
  if (r.findings.length) {
    ...
    for (const f of r.findings) {
      ...
      L.push(
        `- **[${f.severity}]**${code}${bucket}${retry} ${f.layer ?? '?'}/${f.kind}: ${f.detail}${f.evidence ? ` — _"${f.evidence}"_` : ''}${f.screenshotUri ? ` ([screenshot](${f.screenshotUri}))` : ''}${next}`,
      );
    }
  }
...
export function toJUnit(r: ReportData): string {
  ...
  lines.push(
    `<testsuite name="${xmlEscape(suiteName)}" tests="${cases.length}" failures="${failures}" skipped="${skipped}" time="${(r.phaseTimings.totalMs ?? 0) / 1000}">`,
  );
